...
def extract_salary_range(description: str | None) -> Tuple[int | None, int | None]:
    ...
    range_pattern = re.compile(rf"(?P<low>{money})\s*(?:-|–|—|to)\s*(?P<high>{money})", re.IGNORECASE)
...
def _requirement_chunks(description: str | None) -> List[str]:
    text = description or ""
    text = re.sub(r"</?(?:li|p|br|div|ul|ol|h[1-6])[^>]*>", "\n", text, flags=re.IGNORECASE)
    text = re.sub(r"<[^>]+>", " ", text)
    return [normalize(c) for c in re.split(r"[\n\r•]|(?<=[.!?])\s+", text) if normalize(c)]
...
def detect_hard_domain_requirements(description: str | None, profile: Dict) -> List[Dict]:
    ...
    for domain, terms in DOMAIN_TERMS.items():
        ...
        for chunk in chunks:
            ...
            explicit_domain_experience = any("experience" in normalize(hit) for hit in domain_hits)
            ...
            findings.append({"domain": domain, "terms": domain_hits, "years": years, "penalty": penalty, "severity": severity, "gate": gate, "evidence": chunk[:260]})
            break
    return findings
...
def score_job(job: Dict, profile: Dict) -> Tuple[int, Dict]:
    ...
    title_score = WEIGHTS["title"] if title_matches else 6 if family_matches else 4
    total += title_score
    ...
    seniority_score = 1 if bad_seniority else WEIGHTS["seniority"] if contains_any(title, seniority_preferences) else 8 if family_matches else 6
    total += seniority_score
    details["seniority"] = {"score": seniority_score, "max": WEIGHTS["seniority"], "warnings": bad_seniority}

    focus_matches = contains_any(combined, profile.get("focus_terms", DEFAULT_FOCUS_TERMS))
    ...
    bonus_matches = contains_any(combined, profile.get("bonus_terms", DEFAULT_BONUS_TERMS))
    ...
    if salary_min is None or salary_max is None:
        ...
        if salary_min is None and inferred_min is not None:
            salary_min, inferred_salary = inferred_min, True
        if salary_max is None and inferred_max is not None:
            salary_max, inferred_salary = inferred_max, True
    ...
    details["salary"] = {"score": 0, "max": 0, "salary_min": salary_min, "salary_max": salary_max, "salary_floor": salary_floor, "below_floor": salary_below_floor, "inferred_from_description": inferred_salary, "scored": False}
    ...
    details["hard_requirements"] = {"penalty": domain_penalty, "findings": hard_domains, "hard_gate": has_hard_gate, "soft_gate": has_soft_gate, "salary_gate": salary_below_floor}
    total = max(0, min(100, int(round(total))))
    verdict = "SKIP" if (has_hard_gate or salary_below_floor) else "APPLY" if total >= 80 else "STRONG CONSIDER" if total >= 65 else "STRETCH" if total >= 50 else "SKIP"
